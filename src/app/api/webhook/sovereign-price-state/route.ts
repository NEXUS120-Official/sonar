// ============================================================
// SONAR — Sovereign Price State Webhook
// ============================================================
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const normalizedRows = (rows as Array<Record<string, unknown>>)
    .filter((r) => typeof r['asset_key'] === 'string' && !!r['asset_key'])
    .map((r) => ({
      signature: `price_state::${String(r['asset_key'])}::${new Date().toISOString()}`,
      source: 'sovereign_price_state',
      raw_json: r,
