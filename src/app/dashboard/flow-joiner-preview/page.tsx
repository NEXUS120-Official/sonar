...
export default async function FlowJoinerPreviewPage() {
  const db = createAdminClient();
  const rows = await getSovereignFlowJoinerPreview(db, 50);

  return (
    <main className="min-h-screen bg-black text-white p-6 md:p-10">
      <div className="mx-auto max-w-7xl space-y-8">
        <div>
          <h1 className="text-3xl font-semibold tracking-tight">Flow Joiner Preview</h1>
          <p className="mt-2 text-sm text-zinc-400">
            Canonical joined intelligence objects built from sovereign movement context.
          </p>
        </div>

        {rows.length === 0 ? (
          <div className="rounded-2xl border border-zinc-800 bg-zinc-950 p-5 text-sm text-zinc-400">
            No joined movement preview rows available yet.
          </div>
        ) : (
          <section className="rounded-2xl border border-zinc-800 bg-zinc-950 p-5">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {rows.map((row) => (
                    <tr key={row.signature} className="border-b border-zinc-900">
                      <td className="py-3 text-zinc-200">{row.signature}</td>
